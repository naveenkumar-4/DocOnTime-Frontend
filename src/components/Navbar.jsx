...
const Navbar = () => {
    ...
    // const [token, setToken] = useState(true) for temporary use without backend part

    const logout = () => {
        console.log("checking logout functionality")
        setToken(false);
    ...
    return (
        <div className='flex items-center justify-between text-sm py-4 mb-5 border-b border-b-[#ADADAD]'>
            ...
            <div className='flex items-center gap-4'>
                ...
                <img onClick={() => setShowMenu(!showMenu)} className='w-8 md:hidden' src={assets.menu_icon} alt="" />
                {/* ------------- Mobile Menu ---------- */}
                <div className={`${showMenu ? 'fixed w-full' : 'h-0 w-0'} md:hidden right-0 top-0 bottom-0 z-20 overflow-hidden bg-white transition-all`}>
                    <div className='flex items-center justify-between px-5 py-6'>
                        <img className='w-36' src={assets.logo} alt="" />
                        <img className='w-7' onClick={() => setShowMenu(!showMenu)} src={assets.cross_icon} alt="" />
                    </div>
